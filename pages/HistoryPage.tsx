        ...
        {activeTab === 'gifts' ? (
          ...
          <>
            {isLoadingTransactions ? (
              ...
              <p className="text-center text-[#94A3B8] py-10">No transactions yet.</p>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  <thead>
                    <tr className="text-xs font-bold uppercase text-[#64748B] border-b border-white/10">
                      <th scope="col" className="pb-4 pl-4">Type</th>
                      <th scope="col" className="pb-4">Amount</th>
                      <th scope="col" className="pb-4">Date</th>
                      <th scope="col" className="pb-4 pr-4">Status</th>
                    </tr>
                  </thead>
                  <tbody>
                    {transactions.map((tx) => (
                      <tr key={tx.id} className="border-b border-white/5 hover:bg-white/5 transition-colors group">
                        <td className="py-6 pl-4 font-medium text-white whitespace-nowrap">
                          {tx.type === 'onramp' ? (
                            <div className="flex items-center gap-2">
                              <span>💰 Added Funds (MoonPay)</span>
                              {tx.creditIssued && (
                                <span className="px-2 py-1 text-xs font-medium text-[#10B981] bg-[#064E3B]/20 border border-[#10B981]/20 rounded-full">
                                  ✨ $5 Credit
                                </span>
                              )}
                            </div>
                        ...
                        <td className="py-6">
                          {tx.type === 'onramp' ? (
                            ...
                            <span className={tx.isFree ? 'text-[#10B981] font-bold' : 'text-white font-bold'}>
                        ...
                        <td className="py-6 pr-4">
                          {tx.type === 'onramp' ? (
                            <span className="px-2 py-1 text-xs font-bold border bg-[#064E3B]/20 text-[#10B981] border-[#10B981]/20 rounded-full">
                              ✓ Completed
                            </span>
                          ) : (
                            <span className={`px-2 py-1 text-xs font-bold border rounded-full ${
                              tx.isFree
                                ? 'bg-[#064E3B]/20 text-[#10B981] border-[#10B981]/20'
                                : 'bg-[#1E293B]/40 text-[#94A3B8] border-white/10'
                            }`}>
